...
function UDPTransport(contact, options) {
  ...
  self._transport=/:/.test(contact.address)? 'udp6': 'udp4';

  self._socket = dgram.createSocket(self._transport, self._handleMessage.bind(this));
  ...
  this._socket.on('error', function(err) {
    ...
    self._socket = dgram.createSocket(self._transport, self._handleMessage.bind(self));
